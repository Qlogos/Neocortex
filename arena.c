...
#if ARENA_OS_WINDOWS
    return (b32)VirtualFree(ptr, (SIZE_T)size, MEM_DECOMMIT);
#else
...
#if ARENA_OS_WINDOWS
    return (b32)VirtualFree(ptr, (SIZE_T)size, MEM_RELEASE);
#else
...
void *arena_push(mem_arena *arena, u64 size, b32 non_zero) {
    ...
    if (new_pos > arena->commit_pos) {
        ...
        u64  commit_sz = new_commit_pos - arena->commit_pos;

        if (!plat_mem_commit(mem, commit_sz)) return NULL;
